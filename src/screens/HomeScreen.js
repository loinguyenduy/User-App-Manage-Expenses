...
const HomeScreen = ({ navigation }) => {
    ...
    const unsubscribeExpenses = onValue(expensesRef, snapshot => {
      ...
      if (data) {
        Object.values(data).forEach(exp => {
          // UPDATE: Chỉ cộng tổng tiền do CHÍNH Staff này đã tiêu
          if (
            exp.claimant === currentUser.fullName &&
            (exp.status === 'Paid' ||
              exp.status === 'Pending' ||
  ...
  }, [currentUser]); // Cập nhật dependency array cho chuẩn
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.titleGroup}>
          <Text style={styles.welcome}>
            WELCOME BACK, {currentUser?.fullName?.toUpperCase() || 'STAFF'}
          </Text>
      ...
      <View style={styles.summaryCard}>
        ...
        <View style={styles.summaryColumn}>
          <Text style={styles.cardLabel}>MY SPENT</Text>
